...
def run_one_workload(model, dataset, left_sens_attribute, right_sens_attribute, test_file,
    ...
    predictions = pd.read_csv("../data/FairEM/" + model + "/" + dataset + "/preds.csv").values.tolist()
    # predictions = [[i] for i in pd.read_csv("../data/FairEM/DeepMatcher/" + dataset + "/" + test_file).label.values]
    test_file = "../data/FairEM/DeepMatcher/" + dataset + "/" + test_file
...
def plot_one_dataset_all_measures(dataset, experiment, idxx):
    if dataset in ['Shoes', 'Cameras']:
        ...
        models = ["DeepMatcher", "Ditto", "GNEM", "HierMatcher", "MCAN", "SVM", \
                  "RuleBasedMatcher", "RandomForest", "NaiveBayes", "LogisticRegression", "LinearRegression", "DecisionTree", "Dedupe"]
    non_neural = {"SVM", "RuleBasedMatcher", "RandomForest", "NaiveBayes", "LogisticRegression", "LinearRegression",
                  "DecisionTree", "Dedupe"}
    ...
    for model in models:
        df = pd.read_csv(
            "../experiments/" + dataset + "/results/" + model + "_results_experiment" + experiment + ".csv")
        # df.sort_values('sens_attr', inplace=True)
    ...
    for model in models:
        df_train = pd.read_csv(
            "../experiments/" + dataset + "/results2/" + model + "_results_experiment" + experiment + ".csv")
    ...
    y = ["accuracy_parity", "statistical_parity",
    ...
    #DBLP-ACM single [7,6]
    #DBLP-Scholar single [6,6]
    #camera, shoes [20,6] [15,6]
    ...
    ax.set_yticklabels([a for a in y_], fontsize=12)
    ...
    #            fontsize="12", bbox_to_anchor=(1, 1), loc='upper left', borderaxespad=0)
    plt.tight_layout()
    plt.savefig("../experiments/" + dataset + "/results/Unfairness_experiment" + experiment + "_" + str(idxx) + ".png",
...
def experiment_one(model, dataset, left_sens_attribute, right_sens_attribute, threshold,
    ...
    save_pandas_csv_if_not_exists(dataframe=df, outname=model + "_results_experiment1.csv",
                                  outdir="../experiments/" + dataset + "/results/")
...
def dataset_experiments(dataset, sens_att, test_file, idxx, threshold=0.2):
    if dataset in ['Shoes', 'Cameras']:
        models = ["DeepMatcher", "Ditto", "GNEM", "HierMatcher", "MCAN", "SVM",
                  "RuleBasedMatcher", "RandomForest", "NaiveBayes", "LogisticRegression", "LinearRegression",
                  "DecisionTree"]
    ...
    for mod in models:
        ...
        experiment_one(model=mod, dataset=dataset,
                       ...
                       test_file=test_file)
        print("-------------------------------------------------")
    plot_one_dataset_all_measures(dataset, "1", idxx=idxx)
    for mod in models:
        print('Model:', mod)
    ...
    plot_one_dataset_all_measures(dataset, "2", idxx=idxx)


# dataset_experiments("iTunes-Amazon", "Genre", test_file="test_others.csv", idxx=2)
# dataset_experiments("DBLP-ACM", "venue", test_file="test_others.csv", idxx=2)
# dataset_experiments("DBLP-Scholar", "ENTRYTYPE", test_file="test_others.csv", idxx=1)
# dataset_experiments("Cricket", "batting_style", test_file="test_others.csv", idxx=1)
dataset_experiments("Shoes", "company", test_file="test_others__.csv", idxx=2)
# dataset_experiments("Cameras", "company", test_file="test_others__.csv", idxx=2)
